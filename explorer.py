...
def main(result_file):
    ...
    uploaded_file = st.file_uploader("Upload your own results file", type="csv")
    ...
    default_models = [
        model
        for model in ["google/ul2", "google/flan-ul2", "bert-base-uncased", "EleutherAI/pythia-12b"]
        if model in st.session_state["models"]
    ]

    if len(default_models) == 0:
        default_models = random.choices(st.session_state["models"], k=4)
    ...
    with tab2:
        default_phenomena = [
            model
            for model in ["negation", "part-of-speech", "binding"]
            if model in st.session_state["models"]
        ]

        if len(default_phenomena) == 0:
            default_phenomena = random.choices(st.session_state["linguistic_phenomena"], k=4)

        st.multiselect(
            label="Select specific linguistic phenomena to analyze",
            options=st.session_state["linguistic_phenomena"],
            key="selected_phenomena",
            default=default_phenomena
        )
    ...
    with tab3:
        default_probing_datasets = [
            model
            for model in ["pos", "xpos", "upos"]
            if model in st.session_state["models"]
        ]

        if len(default_probing_datasets) == 0:
            default_probing_datasets = random.choices(st.session_state["probing_datasets"], k=4)

        st.multiselect(
            label="Select datasets to analyze",
            options=st.session_state["probing_datasets"],
            key="selected_datasets",
            default=default_probing_datasets
        )
