...
@click.command()
@click.option('--holmes_result_file', type=str, default="data/holmes_results_f1.csv")
@click.option('--flash_holmes_result_file', type=str, default="data/holmes_results_f1_raw_free.csv")
def main(holmes_result_file, flash_holmes_result_file):
    ...
    if "data" not in st.session_state:
        st.session_state["raw_data"] = {
            "Holmes 🔎 - Our comprehensive version": read_data(holmes_result_file),
            "FlashHolmes ⚡ - Our streamlined version, built with efficiency in mind and relying on freely available data": read_data(flash_holmes_result_file, train_portions=[0.03125]),
